...
static void dev_log_save() {
    ...
    for (int i = 0; i < dev_log.count; i++) {
        const char *prefix = dev_log.msgs[i].from_dev ? "[DEV] " : "[YOU] ";
...
static void setup_touch_style(float dpi_scale) {
    ...
    s.FramePadding    = ImVec2(16 * dpi_scale, 12 * dpi_scale);
    s.ItemSpacing     = ImVec2(12 * dpi_scale, 10 * dpi_scale);
    s.ItemInnerSpacing = ImVec2(10 * dpi_scale, 8 * dpi_scale);
    s.TouchExtraPadding = ImVec2(8 * dpi_scale, 8 * dpi_scale);
    s.ScrollbarSize   = 20 * dpi_scale;
    s.GrabMinSize     = 18 * dpi_scale;
    s.WindowPadding   = ImVec2(16 * dpi_scale, 16 * dpi_scale);
    s.WindowRounding  = 8 * dpi_scale;
    s.FrameRounding   = 6 * dpi_scale;
    s.GrabRounding    = 4 * dpi_scale;
    s.ScrollbarRounding = 6 * dpi_scale;
    s.TabRounding     = 6 * dpi_scale;
...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *win = SDL_CreateWindow("Quest & Glory", 800, 600,
    ...
    dev_log_load();
    if (load_game(&game)) {
        // loaded successfully — stay on whatever screen was saved
        // but reset transient combat state if we were mid-combat
        if (game.screen == SCR_COMBAT) game.screen = SCR_TOWN;
    }
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            // save on Android lifecycle events
        ...
        ImGui::Begin("QA Tools", &qa.panel_open, wf_qa);
        {
            ...
            ImGui::SameLine(qa_bw * 0.7f);
            int open_bugs = 0;
            for (int i = 0; i < qa.count; i++) if (!qa.bugs[i].completed) open_bugs++;
            ImGui::Text("Bugs: %d open / %d total", open_bugs, qa.count);
