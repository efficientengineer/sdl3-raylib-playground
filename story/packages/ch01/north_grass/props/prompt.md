# ChatGPT package: north_grass — 2 props

## What exists already

2 of 2 files in this package have been cut already.

- slot 1 `story/field/props/cave_mouth.png` — **exists**
- slot 2 `story/field/props/slide_rubble.png` — **exists**

## 1. Start a new chat and attach these files, in this order

1. `story/packages/ch01/north_grass/props/template.png`
...
````
Create ONE image: the attached template with all 2 numbered slots filled in. Canvas: portrait, 1024x1536, the same size as the template.
...
SLOTS:
Slot 1 (cave_mouth), 3 x 2 cells, 913x607 px in the template: A low dark opening in broken hillside rock, the lip shored with one fallen slab, a rope tied off round a boulder and hanging into it. Front three-quarter view from above, standing on the bottom edge of the slot, magenta on every other side.
Slot 2 (slide_rubble), 3 x 2 cells, 913x607 px in the template: A fan of fresh hillside rubble spilled across a path, raw earth at its top edge, boulders and snapped roots piled at the bottom. Front three-quarter view from above, standing on the bottom edge of the slot, magenta on every other side.
...
`story/packages/ch01/north_grass/props/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/ch01/north_grass/props
```
