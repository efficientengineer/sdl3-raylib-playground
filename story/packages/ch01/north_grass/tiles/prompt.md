# ChatGPT package: north_grass — 2 tiles

## What exists already

2 of 2 files in this package have been cut already.

- slot 1 `story/field/tiles/grass_tall.png` — **exists**
- slot 2 `story/field/tiles/scree.png` — **exists**

## 1. Start a new chat and attach these files, in this order

1. `story/packages/ch01/north_grass/tiles/template.png`
...
SLOTS:
Slot 1 (grass_tall), ground tile, 685x685 px in the template: Shoulder-high grass seen straight down onto the heads, dense and all leaning one way, with narrow dark gaps between the clumps. Seen straight down from directly above, seamless on all four edges.
Slot 2 (scree), ground tile, 685x685 px in the template: Loose broken hill stone in flat shards over dry earth, tipped at every angle, nothing growing in it. Seen straight down from directly above, seamless on all four edges.
...
`story/packages/ch01/north_grass/tiles/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/ch01/north_grass/tiles
```

That writes the 2 tile file(s) listed above. Running `./story_prompt.py ingest` with no path does every package in
