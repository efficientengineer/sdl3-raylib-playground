# ChatGPT package: bridge — 1 props

## What exists already

0 of 1 files in this package have been cut already.

- slot 1 `story/field/props/bridge_rail.png` — missing

## 1. Start a new chat and attach these files, in this order

1. `story/packages/ch01/bridge/props/template.png`
...
SLOTS:
Slot 1 (bridge_rail), 2 x 1 cells, 1425x709 px in the template: A run of low bridge parapet in cut stone with a flat coping, one section rebuilt in newer blocks and a rubbed hollow where hands go. Front three-quarter view from above, standing on the bottom edge of the slot, magenta on every other side.
...
`story/packages/ch01/bridge/props/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/ch01/bridge/props
```
