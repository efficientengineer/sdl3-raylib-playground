# ChatGPT package: valley — props_2 tile sheet (2 of 3)

## What exists already

- `story/field/tilesets/valley/atlas.png` — not started yet
- `story/field/tilesets/valley/cut/props_2.png` — not cut yet

This sheet carries 2 of the tileset's entries: `barn`, `grain_shed`

## 1. Start a new chat and attach these files, in this order

1. `story/packages/tilesets/valley/props_2/template.png`
2. `story/refs/style.png`

## 2. Paste this prompt exactly

````
Create ONE image: the attached template with all 2 numbered slots filled in. Canvas: landscape, 1536x1024, exactly the same size and proportions as the template.
...
WHAT THIS SHEET IS: the 'props_2' tile sheet for the 'valley' tileset of a top-down 16-bit JRPG field map. Every slot is one tile or one multi-tile object, and in game each tile is exactly 32x32 pixels.
...
SLOTS — 2 of them:
Slot 1 (barn), 512x384 px, 4x3 tiles: a barn with tall boarded doors and a hoist beam under the gable. Standing on the slot's bottom edge, everything else left as background.
Slot 2 (grain_shed), 512x384 px, 4x3 tiles: a grain store on stone staddles, the boards tight, one small hatch high in the wall. Standing on the slot's bottom edge, everything else left as background.
...
- [ ] All 2 slots filled; every border and every number exactly where it was
...
`story/packages/tilesets/valley/props_2/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/tilesets/valley/props_2
```

That writes the 2 tile(s) into `story/field/tilesets/valley/atlas.png`. Running `./story_prompt.py ingest` with no path does every package in
