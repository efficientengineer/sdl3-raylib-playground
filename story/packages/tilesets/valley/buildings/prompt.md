# ChatGPT package: valley — buildings tile sheet (1 of 3)

## What exists already

- `story/field/tilesets/valley/atlas.png` — not started yet
- `story/field/tilesets/valley/cut/buildings.png` — not cut yet

This sheet carries 8 of the tileset's entries: `wall_ew`, `wall_ns`, `wall_corner`, `gate_post`, `house_a`, `house_b`, `hut`, `house_c`

## 1. Start a new chat and attach these files, in this order

1. `story/packages/tilesets/valley/buildings/template.png`
2. `story/refs/style.png`

## 2. Paste this prompt exactly

````
Create ONE image: the attached template with all 8 numbered slots filled in. Canvas: landscape, 1536x1024, exactly the same size and proportions as the template.
...
WHAT THIS SHEET IS: the 'buildings' tile sheet for the 'valley' tileset of a top-down 16-bit JRPG field map. Every slot is one tile or one multi-tile object, and in game each tile is exactly 32x32 pixels.
...
SLOTS — 8 of them:
Slot 1 (wall_ew), 128x128 px, 1x1 tile: a dry stone wall a little above waist height running east and west, capped with flat stones. Standing on the slot's bottom edge, everything else left as background.
Slot 2 (wall_ns), 128x128 px, 1x1 tile: the same dry stone wall running north and south. Standing on the slot's bottom edge, everything else left as background.
Slot 3 (wall_corner), 128x128 px, 1x1 tile: the corner of a dry stone wall, the cap stones turning. Standing on the slot's bottom edge, everything else left as background.
Slot 4 (gate_post), 128x128 px, 1x1 tile: one squared stone gate post standing where a wall opens. Standing on the slot's bottom edge, everything else left as background.
Slot 5 (house_a), 384x384 px, 3x3 tiles: a small valley house, plastered walls on a stone footing, red pantile roof, one door and one shuttered window on the south face. Standing on the slot's bottom edge, everything else left as background.
Slot 6 (house_b), 384x384 px, 3x3 tiles: a second small house, timbers showing in the plaster, the roof a shade darker. Standing on the slot's bottom edge, everything else left as background.
Slot 7 (hut), 384x384 px, 3x3 tiles: the smallest house in the town, one room, turf growing on the roof edge. Standing on the slot's bottom edge, everything else left as background.
Slot 8 (house_c), 512x384 px, 4x3 tiles: a wider house with two windows and a bench beside the door. Standing on the slot's bottom edge, everything else left as background.
...
- [ ] All 8 slots filled; every border and every number exactly where it was
...
`story/packages/tilesets/valley/buildings/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/tilesets/valley/buildings
```

That writes the 8 tile(s) into `story/field/tilesets/valley/atlas.png`. Running `./story_prompt.py ingest` with no path does every package in
