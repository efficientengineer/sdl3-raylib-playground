# ChatGPT package: valley — buildings_2 tile sheet (2 of 3)

## What exists already

- `story/field/tilesets/valley/atlas.png` — not started yet
- `story/field/tilesets/valley/cut/buildings_2.png` — not cut yet

This sheet carries 2 of the tileset's entries: `house_d`, `ladder_house`

## 1. Start a new chat and attach these files, in this order

1. `story/packages/tilesets/valley/buildings_2/template.png`
...
WHAT THIS SHEET IS: the 'buildings_2' tile sheet for the 'valley' tileset of a top-down 16-bit JRPG field map. Every slot is one tile or one multi-tile object, and in game each tile is exactly 32x32 pixels.
...
SLOTS — 2 of them:
Slot 1 (house_d), 512x384 px, 4x3 tiles: a house with an outside stair to an upper door and washing lines at the side. Standing on the slot's bottom edge, everything else left as background.
Slot 2 (ladder_house), 512x512 px, 4x4 tiles: a two-storey house with a ladder left leaning against the eaves and slates missing off the ridge. Standing on the slot's bottom edge, everything else left as background.
...
`story/packages/tilesets/valley/buildings_2/returned.png`. A .jpg or .webp works too; the tool converts it.
