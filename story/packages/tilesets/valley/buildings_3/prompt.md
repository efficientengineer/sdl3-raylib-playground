# ChatGPT package: valley — buildings_3 tile sheet (3 of 3)

## What exists already

- `story/field/tilesets/valley/atlas.png` — not started yet
- `story/field/tilesets/valley/cut/buildings_3.png` — not cut yet

This sheet carries 1 of the tileset's entries: `house_e`

## 1. Start a new chat and attach these files, in this order

1. `story/packages/tilesets/valley/buildings_3/template.png`
2. `story/refs/style.png`

## 2. Paste this prompt exactly

````
Create ONE image: the attached template with all 1 numbered slots filled in. Canvas: landscape, 1536x1024, exactly the same size and proportions as the template.
...
WHAT THIS SHEET IS: the 'buildings_3' tile sheet for the 'valley' tileset of a top-down 16-bit JRPG field map. Every slot is one tile or one multi-tile object, and in game each tile is exactly 32x32 pixels.
...
SLOTS — 1 of them:
Slot 1 (house_e), 512x512 px, 4x4 tiles: the tallest house of the town, three windows on the south face, a chimney at the west gable. Standing on the slot's bottom edge, everything else left as background.
...
`story/packages/tilesets/valley/buildings_3/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/tilesets/valley/buildings_3
```
