# ChatGPT package: Garbe — walk sheet

## What exists already

- `story/field/walkers/garbe.png` — missing

## 1. Start a new chat and attach these files, in this order

1. `story/packages/cast/garbe/walker/template.png`
...
Image 3: CHARACTER reference for Garbe. The walker is this character: keep the face, hair, outfit and colours identical to this image in every frame. Use it for the design only; ignore its background and its three-panel layout.
...
`story/packages/cast/garbe/walker/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/cast/garbe/walker
```

That writes `story/field/walkers/garbe.png`. Running `./story_prompt.py ingest` with no path does every package in
