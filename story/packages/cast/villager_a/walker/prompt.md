...
- `story/field/walkers/villager_a.png` — **exists**, 384x576
...
WHAT THIS IS: walking sprite sheets for villager_a in a top-down field map, cut out against the flat magenta. The magenta is empty space, not a backdrop: it runs right up to the figure on every side. No ground, no shadow, no scenery, no props that are not part of the costume.

SLOTS 1-9 ARE VILLAGER_A. a middle-aged woman in a faded blue kerchief, a long brown skirt and a grey shawl, a rolled bed roped across her back
...
That writes `story/field/walkers/villager_a.png`. Running `./story_prompt.py ingest` with no path does every package in
