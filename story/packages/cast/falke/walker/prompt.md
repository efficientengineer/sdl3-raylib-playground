...
- `story/field/walkers/falke.png` — missing
...
WHAT THIS IS: a walking sprite sheet for Bron in a 2.5D field map, 16 frames of one character, cut out against the flat magenta. The magenta is empty space, not a backdrop: it runs right up to the figure on every side. No ground, no shadow, no scenery, no props that are not part of the costume.
...
That writes `story/field/walkers/falke.png`, the 4x4 walk sheet. Running `./story_prompt.py ingest` with no path does every package in
