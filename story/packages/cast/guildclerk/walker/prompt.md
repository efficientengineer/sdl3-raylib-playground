# ChatGPT package: Guildclerk — walk sheet

## What exists already

- `story/field/walkers/guildclerk.png` — missing

## 1. Start a new chat and attach these files, in this order

1. `story/packages/cast/guildclerk/walker/template.png`
...
Image 3: CHARACTER reference for Guildclerk. The walker is this character: keep the face, hair, outfit and colours identical to this image in every frame. Use it for the design only; ignore its background and its three-panel layout.
...
WHAT THIS IS: walking sprite sheets for Guildclerk in a top-down field map, cut out against the flat magenta. The magenta is empty space, not a backdrop: it runs right up to the figure on every side. No ground, no shadow, no scenery, no props that are not part of the costume.

SLOTS 1-9 ARE GUILDCLERK. Guildclerk, a slight upright young man with mousy brown hair combed flat in a hard side parting, small round grey eyes, a narrow clean-shaven face, a high-collared charcoal tunic buttoned to the throat over a white shirt with cuffed sleeves, a plain grey sash, ink on the first two fingers of the right hand, a pen behind one ear, flat black shoes.
...
`story/packages/cast/guildclerk/walker/returned.png`. A .jpg or .webp works too; the tool converts it.
...
That writes `story/field/walkers/guildclerk.png`. Running `./story_prompt.py ingest` with no path does every package in
