...
- reference sheet `story/refs/hart.png` — **exists**
- dialogue portrait `story/portraits/hart.png` — **exists**
...
````
Create ONE image: a character reference sheet for Hart. Canvas: landscape, 1536x1024.
...
- Regenerate until you like the design. This image becomes Hart's look in every scene,
...
`story/packages/cast/hart/refsheet/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/cast/hart/refsheet
```

That writes `story/refs/hart.png` and the dialogue-box portrait cut out of it. Running `./story_prompt.py ingest` with no path does every package in
