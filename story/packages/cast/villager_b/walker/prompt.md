# ChatGPT package: villager_b — walk sheet (no cast entry)

## What exists already

- `story/field/walkers/villager_b.png` — missing

## 1. Start a new chat and attach these files, in this order

1. `story/packages/cast/villager_b/walker/template.png`
...
WHAT THIS IS: a walking sprite sheet for villager_b in a 2.5D field map, 16 frames of one character, cut out against the flat magenta. The magenta is empty space, not a backdrop: it runs right up to the figure on every side. No ground, no shadow, no scenery, no props that are not part of the costume.
...
CHARACTER: a broad older man in a rust-red coat with the sleeves rolled past the elbow, cropped grey hair, a grain sack over one shoulder
...
`story/packages/cast/villager_b/walker/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/cast/villager_b/walker
```

That writes `story/field/walkers/villager_b.png`, the 4x4 walk sheet. Running `./story_prompt.py ingest` with no path does every package in
