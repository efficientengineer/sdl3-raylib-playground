...
- `story/field/walkers/villager_b.png` — **exists**, 384x576
...
WHAT THIS IS: walking sprite sheets for villager_b in a top-down field map, cut out against the flat magenta. The magenta is empty space, not a backdrop: it runs right up to the figure on every side. No ground, no shadow, no scenery, no props that are not part of the costume.

SLOTS 1-9 ARE VILLAGER_B. a broad older man in a rust-red coat with the sleeves rolled past the elbow, cropped grey hair, a grain sack over one shoulder
...
That writes `story/field/walkers/villager_b.png`. Running `./story_prompt.py ingest` with no path does every package in
