# ChatGPT package: linde — walk sheet (no cast entry)

## What exists already

- `story/field/walkers/linde.png` — missing

## 1. Start a new chat and attach these files, in this order

1. `story/packages/cast/linde/walker/template.png`
...
WHAT THIS IS: walking sprite sheets for linde in a top-down field map, cut out against the flat magenta. The magenta is empty space, not a backdrop: it runs right up to the figure on every side. No ground, no shadow, no scenery, no props that are not part of the costume.

SLOTS 1-9 ARE LINDE. a small round-shouldered woman of sixty in a moss-green overdress over a cream underlayer, white hair pinned up under a dark headscarf, a wide canvas apron with deep pockets, a satchel of stoppered bottles on a long strap
...
`story/packages/cast/linde/walker/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/cast/linde/walker
```

That writes `story/field/walkers/linde.png`. Running `./story_prompt.py ingest` with no path does every package in
