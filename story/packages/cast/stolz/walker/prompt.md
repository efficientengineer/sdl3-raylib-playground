...
Image 3: CHARACTER reference for Stolz. The walker is this character: keep the face, hair, outfit and colours identical to this image in every frame. Use it for the design only; ignore its background and its three-panel layout.
...
That writes `story/field/walkers/stolz.png`. Running `./story_prompt.py ingest` with no path does every package in
