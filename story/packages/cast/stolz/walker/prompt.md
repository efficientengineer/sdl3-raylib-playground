# ChatGPT package: Stolz — walk sheet

## What exists already

- `story/field/walkers/stolz.png` — missing

## 1. Start a new chat and attach these files, in this order

1. `story/packages/cast/stolz/walker/template.png`
2. `story/refs/style.png`
3. `story/refs/stolz.png`
...
Image 3: CHARACTER reference for Stolz. The walker is this character: keep the face, hair, outfit, and colors identical to this image in every frame. Use it for the design only; ignore its background and its three-panel layout.
...
WHAT THIS IS: a walking sprite sheet for Stolz in a 2.5D field map, 16 frames of one character, cut out against the flat magenta. The magenta is empty space, not a backdrop: it runs right up to the figure on every side. No ground, no shadow, no scenery, no props that are not part of the costume.
...
`story/packages/cast/stolz/walker/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/cast/stolz/walker
```

That writes `story/field/walkers/stolz.png`, the 4x4 walk sheet. Running `./story_prompt.py ingest` with no path does every package in
