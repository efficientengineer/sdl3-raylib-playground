# ChatGPT package: Stolz — reference sheet

## What exists already

- reference sheet `story/refs/stolz.png` — missing
- dialogue portrait `story/portraits/stolz.png` — missing
...
````
Create ONE image: a character reference sheet for Stolz. Canvas: landscape, 1536x1024.
...
- Regenerate until you like the design. This image becomes Stolz's look in every scene,
...
`story/packages/cast/stolz/refsheet/returned.png`. A .jpg or .webp works too; the tool converts it.

Then cut it up, from the repository root:

```
./story_prompt.py ingest story/packages/cast/stolz/refsheet
```

That writes `story/refs/stolz.png` and the dialogue-box portrait cut out of it. Running `./story_prompt.py ingest` with no path does every package in
