...
| `{{FATHER}}` | Rabe | {{HERO}}'s father. Hunter, walked the water out with {{MENTOR}} for twenty years, died on the road eleven years ago. Never appears; named by {{MENTOR}}, the shrine and the coat on the hook. |
...
| `{{TOWN_HEALER}}` | Linde | The healer of {{HOME_TOWN}}, sixty. Took {{HEALER}} in at nine and has taught her since; decides when she gets full standing. Linde = lime tree. |
...
costing the story one of its sixteen. Every creature has a **singular** row and a `_PL` row, because
the table has no plural mechanism — the text writes `a {{ROAD_CREATURE}}` and `two {{ROAD_CREATURE_PL}}`.
The value never carries an article. `BESTIARY.md` is the long description of each one.
...
| `{{THIEF_CREATURE}}` | thumb | Yellow hopping lump that takes an item out of your pack and runs for the ditch. |
...
| `{{ROAD_CREATURE}}` | drape | Pale blue skin lying flat in the grass that comes up and wraps whoever walks over it. Was "sheet" until the job sheets on the guild board took the word. |
...
| `{{LOOT_TEACHING_FIND}}` | weight | Chapter one's teaching find, off the grain scales. Doubles one attack. |
| `{{LOOT_YARD}}` | brace | Tin under {{MENTOR}}'s eaves. The wearer cannot be held or wrapped. |
| `{{LOOT_ROAD}}` | tooth | In the hoard under the broken culvert. Critical chance. |
...
- The creature and item rows are common words, not spent names: renaming a `{{ROAD_CREATURE}}` costs nothing. Sixteen named *things* exist in the whole game; the two minor villagers are the only ones that may be cut without a rewrite. Adding a fifteenth is a decision, not a convenience.
