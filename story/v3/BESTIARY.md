...
Eight creatures, grouped by where they live. Every one of them is a **token** — the common word the
valley uses, held in `NAMES.md` with a singular and a `_PL` row, so a rename is one line. The road
creature was called "sheet" until the guild's job sheets took the word; it is now
{{ROAD_CREATURE}}. None of them is a real animal with a new head. The planet has been
...
**Lives on.** Grain, sacks, seed stores. A shed with {{GRAIN_CREATURE_PL}} in it is a shed with no harvest in it.
...
### {{ROAD_CREATURE_PL}}
**Silhouette.** A man-sized piece of pale blue skin lying flat over the grass, which lifts at one
...
**Drops.** The {{LOOT_SECOND_SWORD}} — worse than {{HERO}}'s father's, and it cannot break. See `LOOT.md` for
what that is for.
...
**The fight.** There is none. {{SHRINE_CREATURE_PL}} do not move and cannot be hurt, and nothing else will cross a
