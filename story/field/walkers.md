...
This file is only for walkers with no cast entry at all: the clerk, the villagers walking out of
Halm. One `## <id>` per walker, with a `- look:` line in the same voice as `characters.md` — what is
visible, no race, class or beard words, no style words.

## clerk
- look: a thin young man in a grey belted tunic with a high collar, dark sleeve guards to the elbow, short flat brown hair, a heavy ledger carried against his chest
