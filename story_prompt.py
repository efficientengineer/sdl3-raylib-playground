...
def cmd_portraits(args):
    ...
    for key, c in cast.items():
        ...
        if not ref:
            print(f"warning: no reference sheet for {c['name']} at {c.get('ref')}; "
                  f"run: story_prompt.py refsheet {c['name']}", file=sys.stderr)
            continue
        w, h, ch, rows = read_png(ref)
        boxes = find_panels(w, h, ch, rows)
        if len(boxes) != 3:
            print(f"warning: {ref.name} yielded {len(boxes)} panel(s), not the reference sheet's 3 "
                  f"(full body | portrait | profile); no portrait written for {c['name']}", file=sys.stderr)
            continue
        ...
        write_png(out, x1 - x0, y1 - y0, ch, [rows[y][x0 * ch:x1 * ch] for y in range(y0, y1)])
        print(f"wrote {out.relative_to(ROOT.parent)}  {x1-x0}x{y1-y0}  (ships as portrait_{key}.png)")
        made += 1
    if not made:
